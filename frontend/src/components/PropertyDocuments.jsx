...
function PropertyDocuments() {
  ...
  // Get user role from localStorage (default to 'Client')
  let userRole = 'Client';
  try {
  ...
  const dashboardRoute =
    ...
    : userRole === 'Estate Agent' ? '/dashboard/estate-agent'
    : '/';
  ...
  const handleAccept = async (fileId) => {
    try {
      const updatedFile = await fileAPI.updateFileReview(fileId, 'accepted');
      setDocuments(prevDocs => 
        prevDocs.map(doc => 
          doc.id === fileId ? { ...doc, review_status: 'ACCEPTED' } : doc
        )
      );
    } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="flex gap-2">
              <Link 
                to={userRole === 'Estate Agent' ? `/estate-agent/property/${propertyId}` : `/property/${propertyId}`} 
                          ...
                          {uploadedFile && editingNotes === uploadedFile.id ? (
                            <div className="flex flex-col gap-2">
                              <textarea
                                ...
                                className="border rounded-md px-3 py-2 text-sm w-full min-h-[70px] resize-vertical"
                                style={{ minHeight: '70px', width: '100%', padding: '0.75rem', fontSize: '1rem', resize: 'vertical' }}
                              />
                        ...
                        <div>
                          {uploadedFile ? (
                            <div className="flex flex-wrap gap-2">
                              ...
                              {doc.requiredBy.split('/').includes(userRole) && (
                                <button
                                  onClick={() => handleDeleteFile(uploadedFile.id)}
                                  className="border rounded-full px-3 py-1 text-sm text-red-700 border-red-400 hover:bg-red-50"
                                >
                                  Delete
                                </button>
                              )}
                              {userRole === 'Solicitor' && doc.requiredBy === 'Buyer' && (
                                <>
                                  {uploadedFile.review_status === 'accepted' && (
                                    <span className="ml-2 text-green-600 text-xs font-semibold">Accepted</span>
                                  )}
                                  {uploadedFile.review_status === 'denied' && (
                                    <span className="ml-2 text-red-600 text-xs font-semibold">Denied</span>
                                  )}
                                  {!uploadedFile.review_status && (
                                    <>
                                      <button
                                        onClick={() => handleAccept(uploadedFile.id)}
                                        className="border rounded-full px-3 py-1 text-sm text-green-700 border-green-600 hover:bg-green-50"
                                      >
                                        Accept
                                      </button>
                                      <button
                                        onClick={() => handleDeny(uploadedFile.id)}
                                        className="border rounded-full px-3 py-1 text-sm text-red-700 border-red-400 hover:bg-red-50"
                                      >
                                        Deny
                                      </button>
                            ...
                            doc.requiredBy.split('/').includes(userRole) ? (
                              <button
                                onClick={() => handleDocumentAction(doc.key, 'upload')}
                                className="border rounded-full px-3 py-1 text-sm text-blue-700 border-blue-600 hover:bg-blue-50"
